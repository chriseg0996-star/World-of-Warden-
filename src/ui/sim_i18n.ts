...
const baseEnTable = {
  ...
  "log.defendStart": "Enemies approach! Defend the position.",
  "log.defendFailed": "The position is overrun! The defense must start again.",
  ...
  "loot.rollChoiceNeed": "{name} has selected Need for {item}.",
  "loot.rollChoiceGreed": "{name} has selected Greed for {item}.",
  "loot.rollChoicePass": "{name} passes on {item}.",
  "loot.rollWinNeed": "{winner} wins {item} with a Need roll of {roll}.",
  "loot.rollWinGreed": "{winner} wins {item} with a Greed roll of {roll}.",
  "loot.rollAllPassed": "Everyone passed on {item}. It goes to {name}.",
  "aura.tamed": "Tamed",
} as const;
...
const BASE_DICT: Record<SupportedLanguage, Record<BaseSimMessageKey, string>> = {
  ...
  es: {
    ...
    "log.defendStart": "¡Se acercan enemigos! Defiende la posición.",
    "log.defendFailed": "¡La posición ha sido arrasada! La defensa debe empezar de nuevo.",
    ...
    "loot.rollChoiceNeed": "{name} ha elegido Necesidad para {item}.",
    "loot.rollChoiceGreed": "{name} ha elegido Codicia para {item}.",
    "loot.rollChoicePass": "{name} pasa de {item}.",
    "loot.rollWinNeed": "{winner} gana {item} con una tirada de Necesidad de {roll}.",
    "loot.rollWinGreed": "{winner} gana {item} con una tirada de Codicia de {roll}.",
    "loot.rollAllPassed": "Todos han pasado de {item}. Se lo queda {name}.",
    "aura.tamed": "Domado",
  },
};
...
const RULES: Rule[] = [
  ...
  { re: /^(.+) has selected Need for (.+)\.$/, build: (m) => tSim('loot.rollChoiceNeed', { name: m[1], item: locItem(m[2]) }) },
  { re: /^(.+) has selected Greed for (.+)\.$/, build: (m) => tSim('loot.rollChoiceGreed', { name: m[1], item: locItem(m[2]) }) },
  { re: /^(.+) passes on (.+)\.$/, build: (m) => tSim('loot.rollChoicePass', { name: m[1], item: locItem(m[2]) }) },
  { re: /^(.+) wins (.+) with a Need roll of (\d+)\.$/, build: (m) => tSim('loot.rollWinNeed', { winner: m[1], item: locItem(m[2]), roll: m[3] }) },
  { re: /^(.+) wins (.+) with a Greed roll of (\d+)\.$/, build: (m) => tSim('loot.rollWinGreed', { winner: m[1], item: locItem(m[2]), roll: m[3] }) },
  { re: /^Everyone passed on (.+)\. It goes to (.+)\.$/, build: (m) => tSim('loot.rollAllPassed', { item: locItem(m[1]), name: m[2] }) },
